...
# ═══════════════════════════════════════════════════════════════════════════════
#              🎨 MODERN ANIMATION & UI SYSTEM
# ═══════════════════════════════════════════════════════════════════════════════

# Cute bunny loading frames
BUNNY_FRAMES = [
    "  <code>(\\/)</code>\n  <code>( •ω•)</code>\n  <b>> {label}...</b> ✨",
    "  <code>(\\/)</code>\n  <code>( ^ω^)</code>\n  <b>> {label}...</b> 💫",
    "  <code>(\\/)</code>\n  <code>( •ω•)</code>\n  <b>> {label}...</b> 🌸",
...
def div(char="·", length=22) -> str:
    """Thin decorative divider."""
    return f"<code>·{'─' * length}·</code>"

def title_line(emoji: str, text: str) -> str:
    """Modern title: emoji + bold text."""
    return f"{emoji} <b>{text}</b>"

def field(label: str, value: str, emoji: str = "▸") -> str:
    return f"{emoji} <b>{label}:</b> {value}"

def progress_bar(value: int, max_val: int, length: int = 10,
...
def level_title(lvl: int) -> str:
    titles = {1: "Newcomer", 5: "Member", 10: "Regular", 15: "Active",
              20: "Veteran", 30: "Legend", 50: "Deity", 100: "Transcendent"}
...
async def send_loading(update: Update, text: str = None) -> Optional[Message]:
    msg_text = text or BUNNY_FRAMES[0].format(label="Working")
    try:
...
async def animate_loading(update: Update, label: str = "Processing") -> Optional[Message]:
    """Multi-frame cute bunny loading animation."""
    frames = [f.format(label=label) for f in BUNNY_FRAMES]
    try:
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not await is_admin(context, update.effective_chat.id, update.effective_user.id):
            await update.message.reply_text(
                "  <code>(\\/)</code>\n  <code>( >ω<)</code>\n\n"
                "🚫 <b>Access Denied!</b>\n"
                "<i>This command needs admin rights.</i>",
                parse_mode="HTML")
            return
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type != "private":
        ...
        await reply(update,
            "  <code>(\\/)</code>\n"
            "  <code>( •ω•)💖</code>\n\n"
            "⚡ <b>Nexus Bot v9.0</b> — online and ready!\n"
            "📖 Use /help · ⚙️ Use /settings",
            reply_markup=kb)
        return
    ...
    text = (
        "  <code>(\\/)</code>\n"
        "  <code>( •ω•)💖</code>\n\n"
        f"✨ <b>Hey {name[:20]}!</b> Welcome to <b>Nexus Bot v9.0</b>\n"
        f"{div()}\n"
        "🛡️ <b>Advanced Moderation</b> — smart ban, mute, warn\n"
        "🚫 <b>Anti-Spam Engine</b> — flood, raid, link, NSFW\n"
        "📝 <b>Notes & Filters</b> — auto-responses & keywords\n"
        "🌐 <b>Federation</b> — cross-group bans\n"
        "💰 <b>Economy + Shop</b> — earn, spend, trade\n"
        "🏆 <b>Leaderboards</b> — rankings & competitions\n"
        "⭐ <b>Levels & Ranks</b> — activity rewards\n"
        "🎮 <b>Games & Fun</b> — trivia, truth, dare\n"
        f"{div()}\n"
        "👇 <b>300+ features — smarter than ever!</b>"
    )
    kb = InlineKeyboardMarkup([
...
async def _warn(update, context, silent=False, delete_msg=False):
    ...
    if not silent:
        ...
        text = (
            ...
            f"▸ <b>Admin:</b> {user_link(update.effective_user)}\n"
            f"▸ <b>Reason:</b> {html.escape(reason or 'No reason')}\n\n"
            f"🔢 <b>Warns:</b> {count}/{warn_limit}  [{bar}]{extra_action}"
        )
...
async def work_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in work_cd and now - work_cd[user_id] < 3600:
        ...
        return await reply(update,
            f"  <code>(\\/)</code>\n"
            f"  <code>( -ω-)</code>\n\n"
            f"💼 <b>Still tired from work!</b>\n"
            f"▸ Rest for <b>{m}m {s}s</b> more!"
        )
    jobs = [
    ...
    await reply(update,
        ...
        f"  <code>( •ω•)💼</code>\n\n"
        f"💼 <b>Work Complete!</b>\n{_D}\n\n"
        f"▸ You {job} and earned <b>{earned:,} 🪙</b>!\n"
        f"<i>Come back in 1 hour for more work!</i>"
    )
