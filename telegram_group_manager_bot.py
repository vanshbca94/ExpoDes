#!/usr/bin/env python3
"""
Nexus Bot v9.0 — Ultra Advanced Telegram Group Manager
Modern · Animated · Cute · Powerful
"""
...
async def animate_loading(update: Update, label: str = "Processing") -> Optional[Message]:
    """Gen Z vibe loading animation."""
    k = kmo(KAOMOJI_VIBE)
    msg1 = f"{k}\n<b>{random.choice(GEN_Z_LOADING)}</b>"
    try:
        m = await update.message.reply_text(msg1, parse_mode="HTML")
        frames = [
            f"(⌛ ω ⌛)\n<b>{random.choice(GEN_Z_LOADING)}</b>",
            f"{kmo(KAOMOJI_THINK)}\n<b>{random.choice(GEN_Z_LOADING)}</b>",
            f"{kmo(KAOMOJI_VIBE)}\n<b>{random.choice(GEN_Z_LOADING)}</b>",
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type != "private":
        ...
        await reply(update,
                        "⚡ <b>nexus bot v9.0 is LIVE no cap!!</b>\n"
            f"📖 Use /help · ⚙️ Use /settings\n"
            f"<i>{random.choice(GEN_Z_PHRASES)} {kmo(KAOMOJI_HYPE)}</i>",
            reply_markup=kb)
        return

    m = await animate_loading(update, "Waking up")
    name = html.escape(update.effective_user.first_name or "Friend")
    text = (
                f"✨ <b>heyyy {name[:20]}!!</b> bestie nexus v9 just hit different fr\n"
        f"{div()}\n"
        "🛡️ <b>Moderation</b> — bans mutes warns (admin slay era)\n"
        "🚫 <b>Anti-Spam</b> — flood raid link NSFW protection fr\n"
        "📝 <b>Notes & Filters</b> — auto-reply king energy bestie\n"
        "🌐 <b>Federation</b> — cross-group bans no cap\n"
        "💰 <b>Economy</b> — earn flex trade ur coins periodt\n"
        "🏆 <b>Leaderboards</b> — who ate? rankings say it all\n"
        "⭐ <b>Levels & Ranks</b> — activity rewards slay\n"
        "🎮 <b>Games & Fun</b> — trivia truths dares main character vibes\n"
        f"{div()}\n"
        f"👇 <b>300+ features. understood the assignment.</b> {kmo(KAOMOJI_HYPE)}"
    )
    kb = InlineKeyboardMarkup([
...
HELP_SECTIONS = {
    "help_main": (
                f"📋 <b>nexus help — the vibes are immaculate</b> ✨ v9.0\n{_D}\n"
        "🔍 <i>pick a category bestie fr:</i>",
        [
            [InlineKeyboardButton("🛡️ Moderation", callback_data="help_mod"),
...
async def welcome_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for user in members:
        ...
        welcome = cfg.get("welcome_msg") or (
            ...
            f"You're member #{count} in <b>{{chatname}}</b>! 🎉"
        )
...
async def filter_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) < 2:
        return await reply(update, "❓ <b>Usage:</b> <code>/filter keyword reply</code>\n"
                           "<i>Regex: /filter regex:pattern reply</i>")
...
async def settings_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🛡️ Protection", callback_data="protect_toggle:__panel__"),
...
async def botstats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    m = await animate_loading(update, "Fetching statistics")
    db = get_db()
...
async def ping_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if m:
        await m.edit_text(
            f"🏓 <b>PONG bestie!</b> {kmo(KAOMOJI_VIBE)}\n{_D}\n\n"
...
async def daily_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.commit(); db.close()
    await reply(update,
                f"🎁 <b>daily coins just dropped!!</b> {kmo(KAOMOJI_MONEY)}\n{_D}\n\n"
        f"💰 <b>+{reward:,} 🪙 secured in the bag fr bestie</b>\n"
        f"📅 <i>come back tmrw. it's giving daily grind slay periodt</i>"
    )
...
async def work_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    jobs = [
        ...
        ("🎨 designed a logo", 80, 250),
        ("📦 delivered packages", 50, 200),
        ("🍕 delivered pizza", 40, 150),
        ("🔧 fixed a bug", 150, 400),
        ("📚 tutored a student", 60, 180),
        ("🎮 tested a game", 90, 270),
        ("🌟 wrote an article", 70, 220),
        ("🚗 drove for rideshare", 60, 200),
        ("🌱 tended the garden", 30, 120),
    ]
    ...
    db.commit(); db.close()
    await reply(update,
                f"💼 <b>bag secured no cap</b> {kmo(KAOMOJI_MONEY)}\n{_D}\n\n"
        f"▸ you {job} and ATE — <b>{earned:,} 🪙</b> fr bestie\n"
        f"⏰ <i>grind again in 1hr. bag mentality periodt slay</i>"
    )
...
async def mine_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    found = random.randint(10, 200)
    gems = {0: "🪨 Stone", 1: "⛏️ Coal", 2: "💎 Diamond!"}
    gem = gems[min(random.choices([0,1,2], weights=[70,25,5])[0], 2)]
    db = get_db()
    db.execute("UPDATE users SET coins=coins+? WHERE user_id=?", (found, user_id))
    db.commit(); db.close()
    await finish_anim(m,
                f"⛏️ <b>mining slay era bestie</b> {kmo(KAOMOJI_MONEY)}\n{_D}\n\n"
        f"▸ Found: <b>{gem}</b> no cap\n"
        f"▸ Secured: <b>+{found:,} 🪙</b> fr fr periodt!"
    )
...
async def coins_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await reply(update,
        f"💰 <b>bag check bestie</b> {kmo(KAOMOJI_MONEY)}\n{_D}\n\n"
        f"▸ <b>User:</b> {user_link(target)}\n"
        f"{_D}\n"
        f"▸ <b>Wallet:</b> {wallet:,} 🪙 {'bussin fr' if wallet > 5000 else 'rip'} \n"
        f"[{bar}]\n"
        f"▸ <b>Bank:</b> {bank:,} 🪙\n"
        f"▸ <b>Total:</b> {wallet+bank:,} 🪙 — {gz()}"
    )
...
async def bank_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or context.args[0] not in ("deposit", "withdraw", "balance"):
        return await reply(update,
            f"🏦 <b>bank bestie — secure the bag fr</b> {kmo(KAOMOJI_MONEY)}\n{_D}\n\n"
            "<code>/bank deposit N</code> — stash ur coins\n"
            "<code>/bank withdraw N</code> — pull the coins out\n"
            "<code>/bank balance</code> — check the situation"
        )
    ...
    if action == "deposit":
        ...
        db.commit(); db.close()
        await reply(update, f"🏦 <b>Deposited {amount:,} coins!</b>\n▸ Wallet: {wallet-amount:,} · Bank: {bank+amount:,}")
    else:
...
async def rob_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not victim or victim["coins"] < 100:
        db.close(); return await finish_anim(m, "❌ <b>Target is broke!</b> (need min 100 🪙)")
...
async def flip_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args or not context.args[0].isdigit():
        ...
        await finish_anim(m,
                        f"🪙 <b>coin flip fr bestie</b> {kmo(KAOMOJI_VIBE)}\n{_D}\n\n"
            f"▸ Result: <b>{result}</b>! it's giving decisive energy"
        )
        return
    ...
    if random.random() > 0.5:
        db.execute("UPDATE users SET coins=coins+? WHERE user_id=?", (amount, user_id))
        result_text = (
                        f"🎉 <b>W. HEADS. SECURED.</b> {kmo(KAOMOJI_WIN)}\n{_D}\n\n"
            f"🦅 <b>HEADS NO CAP!</b>\n▸ <b>+{amount:,} 🪙</b> in the bag fr fr"
        )
    else:
        db.execute("UPDATE users SET coins=coins-? WHERE user_id=?", (amount, user_id))
        result_text = (
                        f"😭 <b>L. tails got you bestie</b> {kmo(KAOMOJI_LOSE)}\n{_D}\n\n"
            f"🪙 <b>TAILS</b> rip ur coins\n▸ <b>-{amount:,} 🪙</b>. that's an L fr"
        )
...
async def slots_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for _ in range(3):
        ...
        try: await m.edit_text(
            f"<b>spinning rn bestie no cap...</b>\n┃ {spin} ┃", parse_mode="HTML")
        except: pass
        await asyncio.sleep(0.35)
    ...
    if roll[0] == roll[1] == roll[2]:
        ...
        outcome    = f"🎉🎉 <b>JACKPOT NO CAP!!!</b> {kmo(KAOMOJI_WIN)}\n▸ +<b>{winnings:,}</b> 🪙 (x{multiplier}) ATE AND LEFT NO CRUMBS FR BESTIE"
        if amount:
            db = get_db()
        ...
        outcome = f"✨ <b>pair!!</b> {kmo(KAOMOJI_VIBE)} bet returned bestie. could be worse ngl"
    else:
        outcome = f"💀 <b>L + ratio</b> {kmo(KAOMOJI_LOSE)} no match. -{amount:,} 🪙 rip. the casino ate YOU fr"
        if amount:
            db = get_db()
            db.execute("UPDATE users SET coins=MAX(0,coins-?) WHERE user_id=?", (amount, user_id))
            db.commit(); db.close()
    await finish_anim(m,
                f"🎰 <b>slot machine said what it said</b> {kmo(KAOMOJI_HYPE)}\n{_D}\n\n"
        f"┃ {result} ┃\n\n{outcome}"
    )
...
async def wyr_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton(opt1, callback_data="wyr:a"),
        InlineKeyboardButton(opt2, callback_data="wyr:b"),
    ]])
    await finish_anim(m,
...
async def slap_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    target = get_target(update, context)
    if not target: return await reply(update, f"❓ reply to someone to slap them bestie {kmo(KAOMOJI_BAN)}")
    msg = random.choice(SLAPS).format(user=user_link(update.effective_user), target=user_link(target))
    caption = f"👋 <b>SLAP!!!</b> {kmo(KAOMOJI_BAN)}\n{_D}\n\n{msg}\n\n<i>they did not see that coming fr ngl lmao</i>"
    await send_gif_reply(update, "slap", caption)

async def hug_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    target = get_target(update, context)
    if not target: return await reply(update, f"❓ reply to someone to hug them bestie {kmo(KAOMOJI_WHOLESOME)}")
    msg = random.choice(HUGS).format(user=user_link(update.effective_user), target=user_link(target))
    caption = f"🤗 <b>aww bestie got a hug!!</b> {kmo(KAOMOJI_WHOLESOME)}\n{_D}\n\n{msg}\n\n<i>wholesome moment fr fr periodt ✨</i>"
    await send_gif_reply(update, "hug", caption)

async def kiss_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    target = get_target(update, context)
    if not target: return await reply(update, f"❓ reply to someone to kiss them bestie {kmo(KAOMOJI_WHOLESOME)}")
    msg = random.choice(KISSES).format(user=user_link(update.effective_user), target=user_link(target))
    caption = f"💋 <b>omg a kiss bestie</b> {kmo(KAOMOJI_WHOLESOME)}\n{_D}\n\n{msg}\n\n<i>the romance arc is REAL fr periodt</i>"
    await send_gif_reply(update, "kiss", caption)

async def pat_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    target = get_target(update, context)
    if not target: return await reply(update, f"❓ reply to someone to pat them bestie {kmo(KAOMOJI_WHOLESOME)}")
    msg = random.choice(PATS).format(user=user_link(update.effective_user), target=user_link(target))
    caption = f"🫶 <b>aww headpat energy</b> {kmo(KAOMOJI_WHOLESOME)}\n{_D}\n\n{msg}\n\n<i>wholesome bestie moment no cap</i>"
    await send_gif_reply(update, "pat", caption)

async def poke_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    target = get_target(update, context)
    if not target: return await reply(update, f"❓ reply to someone to poke them bestie {kmo(KAOMOJI_VIBE)}")
    msg = random.choice(POKES).format(user=user_link(update.effective_user), target=user_link(target))
    caption = f"👉 <b>gotcha poked lmao</b> {kmo(KAOMOJI_VIBE)}\n{_D}\n\n{msg}"
...
async def ship_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await finish_anim(m,
                f"💕 <b>shipping these besties rn bestie</b> {kmo(KAOMOJI_WHOLESOME)}\n{_D}\n\n"
        f"▸ {user_link(u1)} ❤️ {n2}\n\n"
        f"[{hearts}]\n"
        f"▸ <b>{compat}% compatible no cap!</b>\n"
        f"<i>{verdict}</i>\n\n<i>{gz()} 💕</i>"
    )
...
async def roast_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await reply(update,
                f"🔥 <b>bestie just got COOKED</b> {kmo(KAOMOJI_FIRE)}\n{_D}\n\n"
        f"▸ {name}:\n<i>{random.choice(ROASTS)}</i>\n\n<b>L + ratio + no cap bestie 💀</b>"
    )
...
async def compliment_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    name   = user_link(target) if target else user_link(update.effective_user)
    await reply(update,
                f"💐 <b>slay bestie u deserve this fr</b> {kmo(KAOMOJI_WHOLESOME)}\n{_D}\n\n"
        f"▸ {name}:\n{random.choice(COMPLIMENTS)}\n\n<i>ate no crumbs periodt ✨</i>"
    )

async def joke_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await reply(update,
                f"😂 <b>ok hear me out bestie</b> {kmo(KAOMOJI_VIBE)}\n{_D}\n\n{random.choice(JOKES)}\n\n<i>ok that was lowkey bussin ngl</i>"
    )

async def fact_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await reply(update,
                f"🧠 <b>brainrot incoming no cap</b> {kmo(KAOMOJI_THINK)}\n{_D}\n\n{random.choice(FACTS)}\n\n<i>ngl that's kinda bussin knowledge fr bestie</i>"
    )

async def quote_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await reply(update,
                f"💬 <b>quote dropped. understood the assignment.</b> {kmo(KAOMOJI_VIBE)}\n{_D}\n\n{random.choice(QUOTES)}\n\n<i>{gz()} ✨</i>"
    )

async def truth_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await reply(update,
                f"💭 <b>oop truth time bestie</b> {kmo(KAOMOJI_THINK)}\n{_D}\n\n🤔 <i>{random.choice(TRUTHS)}</i>\n\n<b>no cap answer fr fr</b>"
    )
