...
# ─── AI HELPER (Google Gemini — free forever, 1500 req/day) ───────────────────
# Get a free API key in 30 seconds: https://aistudio.google.com/app/apikey
# Then add it as a Replit secret named  GEMINI_API_KEY

_GEMINI_API_BASE = (
    "https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key={key}"
)
# Model priority: fast tiny model first, escalate on 429/404
_AI_MODELS = [
    "gemma-3-1b-it",       # fastest, lowest quota usage
    "gemma-3-4b-it",       # slightly bigger, still very free
    "gemini-2.0-flash-lite",  # Google's flash-lite, higher quota
    "gemini-2.0-flash",    # full flash as last resort
]

async def ai_reply(prompt: str, fallback: str = "", timeout: float = 10.0,
                   max_tokens: int = 200) -> str:
    """
    Generate AI text using Google Gemini API (free tier).
    Tries models in order from lightest to heaviest — falls back to `fallback`
    only if every model fails or the key is not set.
    """
    ...
    async with aiohttp.ClientSession() as session:
        for model in _AI_MODELS:
            url = _GEMINI_API_BASE.format(model=model, key=GEMINI_API_KEY)
            try:
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        logger.debug(f"ai_reply: {model} rate-limited, trying next")
                        continue
                    else:
                        logger.debug(f"ai_reply: {model} returned {resp.status}")
                        continue
            except Exception as ex:
                logger.debug(f"ai_reply [{model}]: {ex}")
                continue
    return fallback
