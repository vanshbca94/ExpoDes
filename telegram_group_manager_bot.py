...
async def ai_reply(prompt: str, fallback: str = "", timeout: float = 6.0,
                   max_tokens: int = 200) -> str:
    """
    Generate AI text using the custom GPT-4 API endpoint (persistent session).
    Appends random Gen Z phrases/emojis for personality.
    Falls back to `fallback` immediately if API is unreachable or slow.
    """
    try:
